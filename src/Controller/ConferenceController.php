...
class ConferenceController extends AbstractController
{
    ...
    {
        return $this->redirectToRoute('homepage', ['_locale' => 'ua']);
    }
    ...
    public function show(Request $request, Conference $conference, CommentRepository $commentRepository, ConferenceRepository $conferenceRepository, NotifierInterface $notifier, string $photoDir)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            // $context необходим для spamChecker, который я не описал ранее, что бы не регаться в стороннем сервисе
            $context = [
            ...
            $notifier->send(new Notification('Thank you fro the feedback; your comment will be posted after moderation', ['browser']));

            return $this->redirectToRoute('conference', ['slug' => $conference->getSlug()]);
        }

        if ($form->isSubmitted()) {
            $notifier->send(new Notification('Can your check your submission? There are some problems with it.', ['browser']));
        }
